...
class Db {
  ...
  saveImage (image, callback) {
    ...
    let tasks = co.wrap(function * (){
      ...
      let created = yield r.db(db).table(images).get(image.id).run(coon)
...
likeImage (id, callback) {
    ...
    let tasks = co.wrap(function * (){
      ...
      let image = yield r.db(db).table('images')get(imageId).update({
        liked: true,
        likes: image.likes + 1
      }).run(coon)
      let created = yield r.db(db).table('image').get(imageId).run(coon)
