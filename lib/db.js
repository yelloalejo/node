...
class Db {
  ...
  connect(callback) {
    ...
    let setup = co.wrap(function*() {
      ...
      if (dbTables.indexOf('images') === -1) {
        ...
        yield r.db(db).table('images').indexCreate('createAt').run(conn)
      }
  ...
  saveImage(image, callback) {
    ...
    let tasks = co.wrap(function*() {
      ...
      let result = yield r.db(db).table('images').insert().run(conn)
  ...
  likeImage(id, callback) {
    ...
    let tasks = co.wrap(function*() {
      ...
      let created = yield r.db(db).table('image').get(imageId).run(conn)
  ...
  getUser (username, callback) {
    ...
    let tasks = co.wrap(function * () {
      let conn = yield connection
      let user = yield r.db(db).table('users').filter({
        username: username
      }).run(conn)
      let result = yield user.toArray()
